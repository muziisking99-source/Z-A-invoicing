-- Unit + case pricing: case_price on products; snapshot both prices + price_basis on lines.
-- Qty and stock remain in units always; price_basis only chooses which price × qty.
...
ALTER TABLE public.invoice_items
  ...
  CHECK (price_basis IN ('unit', 'case'));
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    v_total := v_total + (v_qty * v_charge);
  END LOOP;

-- Case selling price × typed case qty (not × units).
-- Unit selling price × cases still bills units (qty × pack × unit price).
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    ...
    -- Case/manual: qty × charge. Unit price with cases qty: units × charge.
    IF v_qty_basis = 'case' AND v_price_basis = 'unit' THEN
